...
float left_last_velocities[2] = {0.0, 0.0};
float right_last_velocities[2] = {0.0, 0.0};

float current_angle[2] = {0.0, 0.0};

float last_sent_angle[2] = {0.0, 0.0};
float last_sent_velocity[2] = {0.0, 0.0};
...
void get_encoder_processed_values()
{
	if (increment[LEFT] != 0)
	{
		...
		left_last_velocities[1] = left_last_velocities[0];
		left_last_velocities[0] = angle_increment_left / (SAMPLE_PERIOD / (double)1000000);

		current_velocity_[LEFT] = digital_filter(left_last_velocities, LEFT);
		last_sent_velocity[LEFT] = current_velocity_[LEFT];
	...
	if (increment[RIGHT] != 0)
	{
		...
		right_last_velocities[1] = right_last_velocities[0];
		right_last_velocities[0] = angle_increment_right/ (SAMPLE_PERIOD / (double)1000000);

		current_velocity_[RIGHT] = digital_filter(right_last_velocities, RIGHT);
		last_sent_velocity[RIGHT] = current_velocity_[RIGHT];
